...
`python3.11 tools/proof2tex.py` builds the **whole-volume proofing copy** (`prototypes/volume-proof.tex`; `--part 1|2|3` for one part) — continuous flow, both layers, not the mirror. `python3.11 tools/transcript2tex.py` builds the **whole-volume mirror** (`prototypes/volume-mirror.tex`, the edition's own folios with the 1853 page as a shoulder-note; `--part N` likewise) and regenerates `prototypes/fragments/`, which the three M2 prototypes still `\input`. Both import their parser and renderer from `proof2tex.py` — **fix a parsing bug once, there**. ⚠ After building the mirror, run `python3.11 tools/transcript2tex.py --fit`. The only line that matters is **whether a Part I Greek page landed on a recto** — that is the mirror breaking. Second leaves in Parts II–III are bulk, not damage (shape ruled 2026-08-05, `EDITION-SHAPE.md` §6a). Printed 256 and 257 are the two Part I spreads still wanting hand-fitting. TeX packages paracol/bidi/zref/auxhook are user-installed from the frozen TL2025 repo (system tlmgr can't cross-install from 2026 CTAN). Cardo covers polytonic Greek + Hebrew; Hebrew needs `\RL{}` (bidi).
