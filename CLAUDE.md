...
`python3.11 tools/proof2tex.py` builds the **whole-volume proofing copy** (`prototypes/volume-proof.tex`; `--part 1|2|3` for one part) — continuous flow, both layers, not the mirror. `python3.11 tools/transcript2tex.py` builds the **whole-volume mirror** (`prototypes/volume-mirror.tex`, one leaf per 1853 page, folios are the 1853's own; `--part N` likewise) and regenerates `prototypes/fragments/`, which the three M2 prototypes still `\input`. Both import their parser and renderer from `proof2tex.py` — **fix a parsing bug once, there**. ⚠ After building the mirror, run `python3.11 tools/transcript2tex.py --fit`: a printed page must occupy exactly one leaf, and an overflow desynchronises verso from recto for the rest of the book. Part I overflows on 2 of 263; Parts II–III on 128 of 168 and that is a **decision Wilson owes** (`EDITION-SHAPE.md` §6a), not a bug to fix. TeX packages paracol/bidi/zref/auxhook are user-installed from the frozen TL2025 repo (system tlmgr can't cross-install from 2026 CTAN). Cardo covers polytonic Greek + Hebrew; Hebrew needs `\RL{}` (bidi).
