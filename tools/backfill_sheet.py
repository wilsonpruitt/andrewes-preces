...
def main():
    ...
    # ⚠ keyed by the RESOLVED value too: a line may carry several references
    # and only some of them continuations. Keying by (page, line) alone listed
    # every reference on a brace-catalogue line as though it were one.
    cont = {(p, l, rs): pr for p, l, pr, rs, *_ in ref_index.CONTINUATIONS}
    ...
    for pg in pages:
        ...
        rows = [(src, l, r) for src in (pg, pg + 1)
                for l, r in idx.get(src, []) if (src, l, r) in cont]
        print(f"\n{'='*72}\nprinted {pg}   (band ## {key})")
        if not rows:
