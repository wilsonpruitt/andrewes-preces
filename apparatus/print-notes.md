...
R: 1 ⚠⚠ **This catalogue also stands in PART I, at printed 154, and the Latin is the same words.** *Caput spinis coronatum · Oculi lacrymis perfusi · Aures conviciis oppletæ · Os felle et aceto potum · Facies sputis fœde illita · Dorsum … exaratum · Collum … depressum · Cor lancea perforatum* — phrase for phrase, in the same order, with the same proof-texts. **What differs is the grammar and therefore the frame**: Part I sets the whole list in the ACCUSATIVE under a large left brace cued *Per*, so it is one long prepositional phrase — *by the head crowned with thorns, by the eyes bathed in tears* — pleading the Passion; here it is NOMINATIVE and numbered, an inventory of what was suffered. **This is a Part I / Part II recension parallel**, the first recorded; the known ones run between Parts II and III. **Neither may be conformed to the other.** Part II divides *Manus et pedes perfossos* into two members and adds the knees and the breast; Part I keeps the bloody sweat at the head of the list and Part II drops it. The divergences are the evidence and they stand.
R: 9 ⚠⚠ **And the two recensions cite the same verse by two different numbers, which settles the Psalter question from inside one text.** Part I prints `[*Psal.* xxii. **17**.]` against the pierced hands and feet; this page prints `[*Psal.* xxii. **16**.]` against the same words. Seventeen is the **Prayer Book** Psalter's number for that verse and sixteen is the **Authorised Version's**. So one catalogue, copied, has had its psalm reference renumbered between recensions — the plainest possible demonstration that **the 1853 does not count from one Psalter**, and it is not an inference from two distant pages but from two printings of a single list. It is why this edition prints the plate's figure wherever it stands and names the system, and never reconciles the two.
