...
**Current front: PART II IN PROGRESS. §§1–5 COMPLETE both layers (printed 267–285).** Part I is complete end-to-end (printed 1–263). Part II sections done in `part2/`, each transcript + line-keyed English, self-checked parity:
...
**NEXT = §6 Sacrificium Vespertinum + Horologium, printed 286–290 = PDF 309–313** (title SACRIFICIUM VESPERTINUM verified at PDF 309; HOROLOGIUM sub-head follows on same page; *Et serva nos* refrain). Then Allegatio (291–298), Confessio Laudis (299–302), etc. — work down STRUCTURE.md §PARS SECUNDA.

## NEXT ACTION

**Offset is stable +23** through the whole main body (verified this session at 274/276/278/280/286). Extract with `pdftoppm -jpeg -r 200 -f 309 -l NNN raw/preces1853.pdf raw/end/p`; **printed 286 = PDF 309** (already extracted; read but not yet transcribed). Re-verify the printed page number by eye every batch (dups resume ~368). Continue the `part2/NN-slug-transcript.md` + `-english.md` file pattern; match the §1–5 files exactly (header para + `<!-- printed N (PDF M) -->` markers + `## Translator's flags`).
...
- 2026-07-17 (Opus 4.8, Part II cont.): **Added §5 Deprecatio** (*Ne perdas*, printed 280–285), both layers, parity 6/6 — the brace/column stress-test (nested braces, 2-/3-col antithetical tables, armour catalogue) all encoded. Verified §6 SACRIFICIUM VESPERTINUM opens printed 286 (PDF 309). NEXT = §6.
